...
app.get('/api/users/:userId/stats', authenticateToken, async (req, res) => {
    try {
        ...
        // Получаем посты из ленты и считаем только сердечки на постах этого пользователя
        const feedPosts = await Post.getFeed(1000, 0); // Загружаем всю ленту
        let totalLikes = 0;
        
        feedPosts.forEach(post => {
            // Считаем сердечки только если пост принадлежит этому пользователю
            if (post.author_id === userId && post.reactions && post.reactions.heart) {
                const heartReactions = post.reactions.heart;
                totalLikes += Array.isArray(heartReactions) ? heartReactions.length : 0;
            }
        });
        ...
        res.json({
            ...
            postsCount: feedPosts.filter(post => post.author_id === userId).length
        });
    } catch (error) {
